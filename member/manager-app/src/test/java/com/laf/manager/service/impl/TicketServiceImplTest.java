package com.laf.manager.service.impl;

import com.laf.manager.querycondition.ticket.TicketSaveCondition;
import org.junit.Assert;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.test.context.junit4.SpringRunner;

@RunWith(SpringRunner.class)
@SpringBootTest
public class TicketServiceImplTest {

    @Autowired
    TicketServiceImpl ticketService;

    @Test
    public void getTicketList() throws Exception {
    }

    @Test
    public void operateTicket() throws Exception {
        TicketSaveCondition condition = new TicketSaveCondition();
//        condition.setHandleStatus(1);
        condition.setTicketId(3);
        int result = ticketService.operateTicket(condition);
        Assert.assertTrue(result > 0);

    }

    @Test
    public void getPoints() throws Exception {

    }

    @Test
    public void removeTicketById() throws Exception {
    }

}
