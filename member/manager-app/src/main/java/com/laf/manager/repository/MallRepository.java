...
public class MallRepository {
    ...
    public Mall selectMallById(final Integer mall_id) {
        final String sql = "select m.mall_id,m.mall_name,m.area,m.address,m.pictures," +
                "m.home,m.mall_phone,m.intro,a.content,m.longitude,m.latitude,m.business_hours," +
                "m.AppID,m.AppSecret" +
    ...
    public int insertMall(final Mall mall) {
    ...
    public int updateMall(final Mall mall) {
        final String sql = "update `T_MALL` set address=?,pictures=?,home=?,mall_phone=?,intro=?," +
                "longitude=?,latitude=?,business_hours=?,AppID=?,AppSecret=? where mall_id=?";


        return jdbcTemplate.update(sql, mall.getAddress(), mall.getPictures(), mall.getHome(),mall.getMall_phone(),
                mall.getIntro_id(),mall.getLongitude(), mall.getLatitude(), mall.getBusiness_hours(),
                mall.getAppId(), mall.getAppSecret(), mall.getMall_id());
    }
    ...
    class MallRowMapper implements RowMapper<Mall> {
        ...
        public Mall mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            if (dbUtils.hasColumn(rs, "intro")) mall.setIntro_id(rs.getInt("intro"));
