...
public class GiftCouponRepository {
    ...
    public int insertGiftCoupon(final ReceiveCouponInfo coupon) {
        final String sql = "insert into `T_GIFT_COUPON_INFO` (coupon_name,create_date,`status`,shops,expiry_date_start,expiry_date_end," +
                "circulation,daily_circulation,verification_type,mall_id,issue_start,issue_end,limited,daily_limited," +
                "price,cost_price,discounted_price,required_points,reg_time_start,reg_time_end,sort_id,receive_method," +
                "keep_verification_of,verification_of,intro,text1,picture)" +
                " values (?,?,1,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        ...
        jdbcTemplate.update(new PreparedStatementCreator() {
            ...
            public PreparedStatement createPreparedStatement(Connection conn) throws SQLException {
                ...
                ps.setLong(10, coupon.getIssue_start());
                ps.setLong(11, coupon.getIssue_end());
                ps.setInt(12, coupon.getLimited());
                ps.setInt(13, coupon.getDaily_limited());
                ps.setBigDecimal(14, coupon.getPrice());
                ps.setBigDecimal(15, coupon.getCost_price());
                if (coupon.getDiscounted_price() == null) ps.setNull(16, Types.DECIMAL);
                else ps.setBigDecimal(16, coupon.getDiscounted_price());
                ps.setInt(17, coupon.getRequired_points());
                ps.setLong(18, coupon.getReg_time_start());
                ps.setLong(19, coupon.getReg_time_end());
                ps.setInt(20, coupon.getSort_id());
                ps.setInt(21, coupon.getReceive_method());
                ps.setInt(22, coupon.getKeep_verification_of());
                ps.setInt(23, coupon.getVerification_of());
                ps.setInt(24, coupon.getIntro_id());
                ps.setString(25, coupon.getText1());
                ps.setString(26, coupon.getPicture());
    ...
    public int updateGiftCoupon(final ReceiveCouponInfo coupon) {
        final String sql = "update `T_GIFT_COUPON_INFO` set coupon_name=?,shops=?,expiry_date_start=?,expiry_date_end=?," +
        ...
        return jdbcTemplate.update(sql, coupon.getCoupon_name(), coupon.getShops(), coupon.getExpiry_date_start(),
                ...
                coupon.getReg_time_end(), coupon.getSort_id(), coupon.getReceive_method(), coupon.getKeep_verification_of(),
                coupon.getVerification_of(), coupon.getIntro_id(), coupon.getText1(), coupon.getPicture(), coupon.getCoupon_id());
    }
     ...
     * @return
     */
    public List<ReceiveCouponInfo> selectGiftCouponInfoList(GiftCouponQueryCondition condition) {
        String sql = "SELECT ci.coupon_id,coupon_name,picture,issue_start,issue_end,circulation,daily_circulation," +
                "receive_method,create_date,sort_id,status,ci.text1,ci.mall_id,verification_of," +
                "(select count(1) from `T_GIFT_COUPON` where coupon_id=ci.coupon_id) as total," +
                "(select count(1) from `T_GIFT_COUPON` where coupon_id=ci.coupon_id AND receive_date <= ? and receive_date >= ?) as daily," +
                "(select count(1) from `T_GIFT_COUPON` where coupon_id=ci.coupon_id AND coupon_status=2) as verification" +
                " FROM `T_GIFT_COUPON_INFO` ci where 1=1";
     ...
     * @return
     */
    public ReceiveCouponInfo selectGiftCouponInfoById(final Integer couponId) {
        String sql = "SELECT coupon_id,coupon_name,shops,intro,picture,verification_type,ci.mall_id,issue_start,issue_end,expiry_date_start,expiry_date_end," +
                ...
                "reg_time_start,reg_time_end,sort_id,status,keep_verification_of,content,use_condition" +
                " FROM `T_GIFT_COUPON_INFO` ci LEFT JOIN `T_ARTICLES` r ON intro=article_id" +
                " WHERE coupon_id=?";
    ...
    public int selectGiftCouponsCount(final GiftCouponQueryCondition condition) {
        final String sql = "select count(1) cnt from `T_GIFT_COUPON_INFO` ci where 1=1";
...
//                " left join `T_VERIFICATION_CLERK` vc on vc.vc_id=vl.vc_id";
    ...
    public List<Coupon> multipleSelectCouponLogs(CouponLogFilterCondition $$) {
        String sql = "select ci.coupon_id,ci.coupon_name,c.crl_id,c.receive_date,c.coupon_status," +
                ...
                "from t_gift_coupon c inner join t_gift_coupon_info ci on c.coupon_id=ci.coupon_id " +
                ...
                "from t_gift_verification_log v inner join t_verification_clerk vc on v.vc_id=vc.vc_id " +
    ...
    public List<ReceiveCouponInfo> selectGiftCouponInfoListForRaffle(GiftCouponQueryCondition condition) {
        String sql = "SELECT coupon_id,coupon_name FROM `T_GIFT_COUPON_INFO` " +
                "WHERE ?>=expiry_date_start and ?<=expiry_date_end AND mall_id=?";

        List<ReceiveCouponInfo> list = jdbcTemplate.query(sql,
                new Object[]{condition.getCurr(), condition.getCurr(), condition.getMallId()},
